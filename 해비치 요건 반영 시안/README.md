...
| 알림섹션 온오프 | 제외 | 배너는 항상 고정 노출 (열고 닫는 상호작용 없음) |
...
| 회사별 테마(멀티테넌트) | 반영 | 대표 컬러 1개만 어드민이 관리 → CSS가 밝은 톤/배경 톤을 자동 계산. 현대캐피탈/해비치 2개 테마로 검증 |
...
| 코너 카드 레이아웃 비교 (1개씩 vs 한 줄 2개) | 시안 검토용 | `corner-list--grid2` 클래스로 두 레이아웃을 같은 CSS 안에서 비교. **날짜 스트립에서 9/1을 선택하면 1개씩, 9/2를 선택하면 한 줄 2개로 바뀌는 임시 데모 장치**(실제 서비스 기능 아님, 기본 선택일은 9/2·2열). 방향 확정되면 정리 예정 |
...
- `style.css`는 컴포넌트 스타일만 `var(--brand-primary)` / `var(--brand-primary-light)` / `var(--brand-primary-bg)` 토큰으로 참조하고, 실제 색상값은 절대 갖지 않는다(현재 파일 안에 하드코딩된 브랜드색 hex 없음).
...
- 각 화면 `<head>`는 `style.css` 다음에 `<link id="themeLink" href="resources/css/theme-hyundaicapital.css">`를 로드한다. 실제 서비스에서는 로그인한 사용자의 소속 회사에 맞는 `theme-*.css`를 서버가 내려주면 된다.
- 이 폴더에서는 코드 하나로 양쪽 테마를 바로 비교할 수 있도록, URL에 `?theme=haevichi`를 붙이면(`common.js`) 데모용으로 테마가 바로 전환된다. 예: `home.html?theme=haevichi`
...
- 서버가 회사별 `theme-*.css`를 어떻게 내려줄지(빌드 시 선택 vs. 로그인 후 동적 링크 교체 vs. 서버사이드 인라인)는 백엔드 논의 필요 — 현재는 클라이언트 쿼리 파라미터로 데모만 구현
...
- 코너 카드 1개씩 vs 한 줄 2개 레이아웃 중 어느 쪽으로 확정할지 — 확정되면 날짜 클릭 비교 데모 장치(`corner-list--grid2`, `weekStrip` 클릭 핸들러)를 정리해야 함
